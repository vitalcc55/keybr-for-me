...
- 2026-09-18 — этап #5 реализован и прошёл повторный профильный read-only review без новых actionable findings. Добавлены типизированный descriptor/policy для personal и standard источников, строгие сохранённые `source`/`limit`, полный personal Word List с явным `all`/first-N, снятие скрытого Guided cap только для personal, согласованный model+word-list loader lifecycle с retry/stale-response guards, сброс progress при смене урока и source/filtered/selected metadata в настройках. Штатные assets и остальные режимы сохранены; focused tests, compile, eslint и lockfile-проверка зелёные. Следующий этап — #6 (адаптивный Guided и typed unavailable-state).
