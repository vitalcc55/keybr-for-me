...
Рабочее решение: скачать архив один раз при подготовке данных, распаковать штатными средствами Windows, передать абсолютный путь к TSV импортёру. Во время тренировки обращаться только к локальным собранным файлам. Другие корпуса, словари и переводчики в первую реализацию не подключать. Отдельного исследования условий распространения в этой задаче нет.

### Подготовка данных

Импортировать весь пригодный пул, не первые N строк: исходная подборка отсортирована по длине. Не ограничивать корпус первым коротким фрагментом файла.
...
ID строить из исходного идентификатора английской записи Tatoeba; при объединении дублирующего английского текста правило выбора ID также детерминировано. Исходный TSV и его сведения об авторах сохранять как исходный материал для воспроизводимости, без обязательного вывода в интерфейс.

Один импортёр создаёт JSON и небольшой manifest/report с URL, checksum исходного файла, версией правил, количеством входных/выходных записей и причинами отбраковки. Не вводить вторую редактируемую копию корпуса. Повторный запуск на том же входе даёт идентичный результат; `--check` ничего не записывает.
...
- `packages/keybr-content-words/lib/data/sentences-en-ru.json` — generated.
- `packages/keybr-content-words/lib/data/sentences-en-ru.manifest.json` — generated metadata/report.
...
```powershell
# $InputFile — абсолютный путь к фактически распакованному TSV.
npm run generate-sentences --workspace=@keybr/generators -- --input "$InputFile"
npm run check-sentences --workspace=@keybr/generators -- --input "$InputFile"
...
Документация: обновить `docs/english-sentences-execplan.md`, кратко дополнить `docs/windows-local-runbook.md` источником данных, импортом, переключением и восстановлением после ошибки корпуса. В финальном отчёте перечислить изменения, реальные counts корпуса, проверки, ограничения и неизменность RU ресурсов.
